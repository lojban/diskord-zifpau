...
function getDefinition(valsi) {
    results = documentStore.filter(function(o) { return o.w == valsi; });
    if (results.length) {
        return results[0].d;
    }
...
(function () {
        ...
        $.fn.fancyTooltip = function () {
            return this.find("[title]").each(function () {
                ...
                $(this).on("mouseover.fancyTooltip", function () {
                    ...
                    position.top -= tooltip.height() + 20;
    ...
    function updateToken(tokens, i, token) {
        try {
            ...
            for (var k=0; k<parse.length; k++) {
                ...
                var url = 'http://vlasisku.lojban.org/vlasisku/' + cleaned;
                var replacement = '<a ' +
                    'class="pemcu-atki" ' +
                    'target="_blank" ' +
    ...
    pencu.prototype.observer = function (e) {
        ...
        messages.fancyTooltip();

    };

    pencu.prototype.load = function () {
        $.getScript("https://rawgit.com/lojban/diskord-zifpau/master/pencu/definitions.js");
        initJQueryPlugins($);
    };
