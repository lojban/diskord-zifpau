...
(function () {
    ...
    var gismu = {
        'dunda': '[donor] dunda [donation] [recipient]',
        'vecnu': '[seller] dunda [goods] [buyer]',
    }

    function getSelpehu (word) {
        if (gismu[word]) {
            var selpehu = $("<a>", {
                href: "http://vlasisku.lojban.org/vlasisku/dunda",
                style: "",
                class: "pencu-atki",
            });
            return selpehu;
    ...
    function initJQueryPlugins($) {
        // jQuery Plugins
        // Make sure that these are set up after jQuery is available, but before we
        // need any of them.
        ...
        $.fn.fancyTooltip = function () {
            return this.filter("[title]").each(function () {
                ...
                $(this).addClass("pencu-fancytooltip").removeAttr("title");
    ...
    pencu.prototype.observer = function (e) {
        ...
        messages.each(function() {
            ...
            $.each(tokens, function(i, token) {
                try {
                    var parse = camxes.parse(el.replace(/[^\w'\.]/g, ''));
                    if (parse.length === 0) {
                        return;
                    }
                    ...
                    for (var k=0; k<parse.length; k++) {
                        ...
                        var replacement = '<a class="pemcu-atki" target="_blank" href="http://vlasisku.lojban.org/vlasisku/' + cleaned + '">' + valsi + '</a>';
