...
var vlaste = require('./vlaste');
var lumci = require ("./lumci");
...
function parseToken(token) {
    ...
    try {
        ...
        for (var i = 0; i < tree.length; i++) {
            ...
            var entry = vlaste.find(valsi.replace("-", ""));
...
function load() {
    log("la pencu ku tolcanci");
    lumci.clean(vlaste.entries);
    log("ba'o lumci le vlaste");
}

function unload() {
    log("la pencu ku canci");
}

function start() {
    log("la pencu ku tolsisti");
}
